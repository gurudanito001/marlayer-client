...
export default function AccessoriesPage() {
  ...
  const loadAccessories = async (pageToFetch: number) => {
    if (pageToFetch === 1) {
      setLoading(true);
    } else {
    ...
    try {
      const response = await fetchGadgetsByCategory("accessories", undefined, pageToFetch, 20);
      ...
      if (pageToFetch === 1) {
        ...
        setAccessories((prev) => [...prev, ...mappedProducts]);
      }
      ...
      <main className="flex-grow max-w-7xl w-full mx-auto px-6 md:px-10 lg:px-12 py-12">
        ...
        {loading ? (
          ...
          <div className="flex flex-col justify-center items-center min-h-[400px] bg-white rounded-3xl border border-[#E2EFEB] p-8 text-center">
            <p className="text-sm font-bold text-[#0D2B1E] mb-1">No modular inventory detected</p>
            <p className="text-xs text-[#416B5C]">Operational hardware catalog is currently recycling resources.</p>
          </div>
        ) : (
          <>
            ...
            <div ref={lastElementRef} className="w-full flex justify-center items-center py-8 mt-6 min-h-[40px]">
              ...
              {!hasMore && accessories.length > 0 && (
                <p className="text-xs font-medium text-[#416B5C]/60 italic">
                  All active modular nodes cataloged.
                </p>
              )}
            </div>
          </>
